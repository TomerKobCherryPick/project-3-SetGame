...
class SetGameViewController: UIViewController {
    ...
    @IBOutlet weak var boardOfCardsView: BoardOfCardsView!
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        game.delegate = self
        boardOfCardsView.delegate = self
        ...
        boardOfCardsView.backgroundColor = #colorLiteral(red: 0, green: 0, blue: 0, alpha: 0)
    }
    override func viewDidLayoutSubviews() {
        boardOfCardsView.updateFrame()
        if boardOfCardsView.cardButtons.count == 0 && !game.isGameOver {
            for _ in 0...11 {
                boardOfCardsView.addCardToView()
            }
        }
    ...
    @objc private func rotateReshuffle(_ recognizer: UIRotationGestureRecognizer) {
        game.reshuffle()
        for index in boardOfCardsView.cardButtons.indices {
            updateViewForCard(button: boardOfCardsView.cardButtons[index], index: index)
        }
    }
    ...
    @IBAction func touchCheat(_ sender: Any) {
        ...
        if possibleMatch.isMatched {
            for index in possibleMatch.arrayOfMatchedCards!{
                boardOfCardsView.cardButtons[index].backgroundColor = #colorLiteral(red: 0.9994240403, green: 0.9855536819, blue: 0, alpha: 1)
            }
        }
    }
...
extension SetGameViewController: SetGameDelegate {
    ...
    func macthedIndicesIfExist(match: Array<Int>?) {
        if match != nil {
            for index in match! {
                boardOfCardsView.cardButtons[index].backgroundColor = #colorLiteral(red: 0.721568644, green: 0.8862745166, blue: 0.5921568871, alpha: 1)
            }
            for button in boardOfCardsView.cardButtons {
...
extension SetGameViewController: BoardOfCardsDelegate {
    ...
    func touchedCard(cardButton: UIButton, index: Int)  {
        ...
        for currentIndex in boardOfCardsView.cardButtons.indices {
            updateViewForCard(button: boardOfCardsView.cardButtons[currentIndex], index: currentIndex)
        }
        scoreLabel.text = "Score: \(game.score)"
    }
}
